...
public sealed class LoginHandshakeConfigurationTests
{
    ...
    public void Constructor_RejectsInvalidTcpPort(int port)
    {
        ArgumentOutOfRangeException exception = Assert.Throws<ArgumentOutOfRangeException>(
            () => new LoginHandshakeConfiguration(IPAddress.Loopback, port, TimeSpan.FromSeconds(1)));
    ...
    public void Constructor_RejectsNullGameServerAddress()
    {
        Assert.Throws<ArgumentNullException>(
            () => new LoginHandshakeConfiguration(null!, 5816, TimeSpan.FromSeconds(1)));
    }
    ...
    public void Constructor_RejectsNonIpv4GameServerAddress()
    {
        ArgumentException exception = Assert.Throws<ArgumentException>(
            () => new LoginHandshakeConfiguration(IPAddress.IPv6Loopback, 5816, TimeSpan.FromSeconds(1)));
    ...
    {
        ArgumentOutOfRangeException exception = Assert.Throws<ArgumentOutOfRangeException>(
            () => new LoginHandshakeConfiguration(IPAddress.Loopback, 5816, TimeSpan.FromMilliseconds(milliseconds)));
