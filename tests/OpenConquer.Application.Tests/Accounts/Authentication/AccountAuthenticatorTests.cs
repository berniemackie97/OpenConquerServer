...
public sealed class AccountAuthenticatorTests
{
    ...
    [Fact]
    public async Task AuthenticateAsync_PersistsSuccessfulLoginInstantAndReturnsCanonicalUsername()
    {
        ...
        Assert.Equal(successfulLoginAt, repository.LastSuccessfulLoginAt);
    ...
    public async Task AuthenticateAsync_CurrentHashPersistenceFailureAbandonsAttempt(bool cancel)
    {
        ...
        Task<AccountAuthenticationResult> operation = authenticator
            .AuthenticateAsync(
                AccountName,
                Password.AsMemory(),
                s_remoteAddress,
    ...
    public async Task AuthenticateAsync_RehashNeededAttemptsOptimisticReplacement()
    {
        ...
        {
            Snapshot = CreateSnapshot(AccountLoginAccess.Allowed),
            RecordSuccessfulLoginResult = true,
        };
    ...
    public async Task AuthenticateAsync_RepeatedPersistenceConflictsRejectAuthentication()
    {
        ...
        {
            Snapshot = CreateSnapshot(AccountLoginAccess.Allowed),
            RecordSuccessfulLoginResult = false,
        };
    ...
    public async Task AuthenticateAsync_InvalidCredentialsNeverReachDependencies(
        string accountName,
        string password
    )
    {
    ...
    private static async Task AssertRejectedBeforeDependencies(
        string accountName,
    ...
    public async Task AuthenticateAsync_NormalizesNameOnly(
        string supplied,
        string expected,
        string password
    )
    ...
    public async Task AuthenticateAsync_RehashConflictRevalidatesCredentialsAndAccess(
        AccountLoginAccess access,
        AccountPasswordVerificationStatus refreshedStatus,
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task AuthenticateAsync_RehashConflictRejectsDeletedOrRecreatedAccount(
        bool recreated
    )
    {
        FakeRepository repository = new()
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task AuthenticateAsync_ReplacementFailureAbandonsAttempt(
    ...
    public async Task AuthenticateAsync_UnsupportedVerifierStatusAbandonsAttempt()
    {
        ...
        FakePasswordHasher verifier = new()
        {
            VerificationStatus = (AccountPasswordVerificationStatus)99,
        };
    ...
    private sealed class FakeRepository : IAccountAuthenticationRepository
    {
        ...
        public bool RecordSuccessfulLoginResult { get; set; } = true;

        public Action? OnRecordSuccessfulLogin { get; set; }
        ...
        )
        {
            ...
            OnRecordSuccessfulLogin?.Invoke();

            return ValueTask.FromResult(RecordSuccessfulLoginResult);
        }
    }
