...
public sealed class GameLoginTicketRedemptionStoreTests : IClassFixture<AccountDatabaseFixture>, IAsyncLifetime
{
    ...
    private static readonly TimeSpan s_defaultExpirationOffset = TimeSpan.FromMinutes(5);
    ...
    public GameLoginTicketRedemptionStoreTests(AccountDatabaseFixture database)
    {
        ...
        _authenticationKeyRing = new GameLoginTicketAuthenticationKeyRing(
            ...
            [
                ...
                new KeyValuePair<ushort, byte[]>(HistoricalVerificationKeyId, _historicalVerificationKey),
            ]);
    ...
    {
        ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new GameLoginTicketRedemptionStore(null!, _authenticationKeyRing));
    ...
    {
        ArgumentNullException exception = Assert.Throws<ArgumentNullException>(() => new GameLoginTicketRedemptionStore(_dataSource, null!));
    ...
    public async Task TryRedeemAsync_WhenSessionUidIsZero_ThrowsArgumentOutOfRangeException()
    {
        ArgumentOutOfRangeException exception = await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            _store.TryRedeemAsync(0, 1, CancellationToken).AsTask());
    ...
    {
        ArgumentOutOfRangeException exception = await Assert.ThrowsAsync<ArgumentOutOfRangeException>(() =>
            _store.TryRedeemAsync(1, 0, CancellationToken).AsTask());
    ...
    public async Task TryRedeemAsync_WhenAlreadyCancelled_ThrowsWithoutConsumingTicket()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() =>
            _store.TryRedeemAsync(sessionUid, authenticationKey, cancellation.Token).AsTask());
    ...
    {
        GameLoginTicketIdentity? identity = await _store.TryRedeemAsync(GenerateNonzeroUInt32(), GenerateNonzeroUInt32(), CancellationToken);
    ...
    public async Task TryRedeemAsync_WhenCredentialsAreCorrect_ConsumesTicketExactlyOnce()
    {
        ...
        GameLoginTicketIdentity? identity = await _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken);
        ...
        GameLoginTicketIdentity? replay = await _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken);
    ...
    public async Task TryRedeemAsync_WhenAuthenticationKeyIsWrong_DoesNotConsumeTicketAndCorrectKeyStillSucceeds()
    {
        ...
        GameLoginTicketIdentity? rejected = await _store.TryRedeemAsync(sessionUid, wrongAuthenticationKey, CancellationToken);

        Assert.Null(rejected);
        Assert.True(await TicketExistsAsync(sessionUid));

        GameLoginTicketIdentity? accepted = await _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken);
    ...
    [Fact]
    public async Task TryRedeemAsync_WhenTicketIsExpired_ReturnsNullWithoutConsumingTicket()
    {
        AccountRecord account = await InsertAccountAsync();
        uint sessionUid = GenerateNonzeroUInt32();
        uint authenticationKey = GenerateNonzeroUInt32();

        await InsertTicketAsync(account, sessionUid, authenticationKey, expiresFromNow: TimeSpan.FromSeconds(-1));
    ...
    [Fact]
    public async Task TryRedeemAsync_WhenTicketExpiresWhileWaitingForRowLock_ReturnsNullWithoutConsumingTicket()
    {
        AccountRecord account = await InsertAccountAsync();
        uint sessionUid = GenerateNonzeroUInt32();
        uint authenticationKey = GenerateNonzeroUInt32();

        await InsertTicketAsync(account, sessionUid, authenticationKey, expiresFromNow: TimeSpan.FromSeconds(5));

        DateTime expiresAtUtc = await ReadTicketExpirationAsync(sessionUid);
        ...
        try
        {
            ...
            redemptionTask = _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken).AsTask();

            await WaitForInnoDbLockWaitAsync("game_login_tickets");

            Assert.False(redemptionTask.IsCompleted);

            await WaitUntilDatabaseUtcAtOrAfterAsync(expiresAtUtc);

            await lockTransaction.RollbackAsync(CancellationToken.None);
            lockTransactionCompleted = true;

            GameLoginTicketIdentity? identity = await redemptionTask.WaitAsync(TimeSpan.FromSeconds(10), CancellationToken);
    ...
    public async Task TryRedeemAsync_WhenTicketUsesHistoricalVerificationKey_RedeemsSuccessfully()
    {
        ...
        byte[] verifier = GameLoginTicketAuthenticationKeyVerifier.Create(_historicalVerificationKey, sessionUid, authenticationKey);

        try
        {
            await InsertProtectedTicketAsync(account.AccountId, account.Username, sessionUid, verifier, HistoricalVerificationKeyId, s_defaultExpirationOffset);
        }
        finally
        {
    ...
    public async Task TryRedeemAsync_WhenVerifierKeyIdIsUnknown_FailsClosedWithoutConsumingTicket()
    {
        ...
        try
        {
            await InsertProtectedTicketAsync(account.AccountId, account.Username, sessionUid, verifier, UnknownVerificationKeyId, s_defaultExpirationOffset);
        }
        finally
        {
            CryptographicOperations.ZeroMemory(verifier);
        ...
        await Assert.ThrowsAsync<KeyNotFoundException>(() => _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken).AsTask());
    ...
    public async Task TryRedeemAsync_WhenPersistedUsernameIsInvalid_FailsClosedWithoutConsumingTicket()
    {
        ...
        try
        {
            await InsertProtectedTicketAsync(account.AccountId, string.Empty, sessionUid, verifier, ActiveVerificationKeyId, s_defaultExpirationOffset);
        }
        finally
        {
            CryptographicOperations.ZeroMemory(verifier);
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken).AsTask());
    ...
    public async Task TryRedeemAsync_ConcurrentCorrectAttemptsProduceExactlyOneWinner()
    {
        ...
        Task<GameLoginTicketIdentity?>[] operations = Enumerable.Range(0, 8)
            .Select(_ => _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken).AsTask())
            .ToArray();
    ...
    public async Task TryRedeemAsync_WhenWrongAndCorrectAttemptsContendForSameTicket_CorrectAttemptWins()
    {
        ...
        try
        {
            ...
            wrongAttempt = _store.TryRedeemAsync(sessionUid, wrongAuthenticationKey, CancellationToken).AsTask();
            correctAttempt = _store.TryRedeemAsync(sessionUid, authenticationKey, CancellationToken).AsTask();
        ...
        finally
        {
            ...
            if (correctAttempt is not null)
            {
                try
                {
                    await correctAttempt.WaitAsync(TimeSpan.FromSeconds(10), CancellationToken.None);
                }
                catch { }
            }
        }
    ...
    public async Task TryRedeemAsync_WhenCancelledWhileWaitingForTicketLock_DoesNotConsumeTicket()
    {
        ...
        try
        {
            ...
            redemptionTask = _store.TryRedeemAsync(sessionUid, authenticationKey, cancellation.Token).AsTask();
    ...
    private async Task InsertTicketAsync(AccountRecord account, uint sessionUid, uint authenticationKey, TimeSpan? expiresFromNow = null)
    {
        byte[] verifier = _authenticationKeyRing.CreateVerifier(sessionUid, authenticationKey);
        ...
        try
        {
            await InsertProtectedTicketAsync(
                ...
                verifier,
                ActiveVerificationKeyId,
                expiresFromNow ?? s_defaultExpirationOffset);
        }
        finally
        {
            CryptographicOperations.ZeroMemory(verifier);
    ...
    private async Task InsertProtectedTicketAsync(uint accountId, string username, uint sessionUid, byte[] authenticationKeyVerifier, ushort authenticationKeyVerifierKeyId, TimeSpan expiresFromNow)
    {
        ...
        command.CommandText = """
            ...
            VALUES
                (@session_uid,
                 @account_id,
                 @username,
                 TIMESTAMPADD(MINUTE, -10, UTC_TIMESTAMP(6)),
        ...
        command.Parameters.Add("@username", MySqlDbType.VarChar, AccountCredentialPolicy.MaximumUsernameLength).Value = username;
    ...
    private async Task<DateTime> ReadTicketExpirationAsync(uint sessionUid)
    {
        await using MySqlConnection connection = await _dataSource.OpenConnectionAsync(CancellationToken);
        await using MySqlCommand command = connection.CreateCommand();
        ...
        DateTime expiresAtUtc = Assert.IsType<DateTime>(await command.ExecuteScalarAsync(CancellationToken));
    ...
    private async Task WaitUntilDatabaseUtcAtOrAfterAsync(DateTime targetUtc)
    {
        const int MaximumPollingAttempts = 750;
        ...
        for (int attempt = 0; attempt < MaximumPollingAttempts; attempt++)
        {
            ...
            databaseUtcNow = DateTime.SpecifyKind(databaseUtcNow, DateTimeKind.Utc);

            if (databaseUtcNow >= targetUtc)
            {
                return;
            }
        ...
        throw new TimeoutException($"MySQL UTC time did not reach expected ticket expiration {targetUtc:O}.");
    }
