...
internal sealed class AccountConfiguration : IEntityTypeConfiguration<AccountRecord>
{
    ...
    public void Configure(EntityTypeBuilder<AccountRecord> builder)
    {
        ...
        builder.Property(account => account.Username).HasColumnName("username").HasMaxLength(AccountCredentialPolicy.MaximumUsernameLength).IsRequired();
        builder.Property(account => account.PasswordHash).HasColumnName("password").HasMaxLength(MaximumPasswordHashLength).IsRequired();
        ...
        builder.Property(account => account.RegistrationOperationId).HasColumnName("registration_operation_id").HasColumnType("char(64)").HasMaxLength(64).HasCharSet("ascii").UseCollation("ascii_bin");
        builder.HasIndex(account => account.Username).IsUnique().HasDatabaseName("UX_accounts_username");
        builder.HasIndex(account => account.RegistrationOperationId).IsUnique().HasDatabaseName("UX_accounts_registration_operation_id");
